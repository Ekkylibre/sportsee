 ...
 * @param {string} props.title Le titre de la valeur nutritionnelle (par exemple, "Calories", "Protéines").
 * @param {number} props.iconIndex L'index de l'icône à afficher.
 * @returns {JSX.Element} Le composant ValueItem.
 */
function ValueItem({ userId, title, iconIndex }) {
  /**
  ...
  useEffect(() => {
    ...
    const getUserData = async () => {
      try {
        ...
        switch (title) {
          case "Calories":
            setValue(`${userData.keyData.calorieCount}kcal`);
            break;
          case "Protéines":
            setValue(`${userData.keyData.proteinCount}g`);
            break;
          case "Glucides":
            setValue(`${userData.keyData.carbohydrateCount}g`);
            break;
          case "Lipides":
            setValue(`${userData.keyData.lipidCount}g`);
            break;
          default:
            setValue('N/A');
        }
  ...
  /**
   * Tableau des icônes des nutriments.
   * @type {JSX.Element[]}
   */
  const nutrientIcons = [
    <svg key="fireIcon" className="fireIcon" width="17" height="20" viewBox="0 0 17 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <svg key="chickenIcon" className="chickenIcon" width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <svg key="appleIcon" className="appleIcon" width="18" height="20" viewBox="0 0 18 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <svg key="burgerIcon" className="burgerIcon" width="20" height="19" viewBox="0 0 20 19" fill="none" xmlns="http://www.w3.org/2000/svg">
